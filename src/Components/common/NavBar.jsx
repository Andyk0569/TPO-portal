...
const NavBar = ({ setProgress }) => {
    ...
    return (
        <div className={` flex sm:relative bg-richblack-900 w-screen relative z-50 h-16 items-center justify-around border-b-[1px] border-b-richblack-700 translate-y-  transition-all duration-500`}>
            <div className='flex w-11/12 max-w-maxContent items-center justify-around'>
            <Link to='/' onClick={() => { dispatch(setProgress(100)) }} className=' flex flex-row text-center text-richblack-25 justify-center'>
                <img src={logo} width={'80%'} height={42} loading='lazy' className="h-12 md:h-16 w-auto m-2 p-2 " />
                <div className=' self-center text-4xl font-bold cursor-pointer'>JJMCOE</div>
            </Link>
